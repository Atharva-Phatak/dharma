...
import clap.pydantic_models as pm
import sys
...
class ZenMLSetup:
    ...
    def register_slack_alerter(self) -> None:
        ...
        slack_channel_id = config.configuration.slack_channel_id
    ...
    def register_project(self) -> None:
        """Register and set the project as active."""
        console.print("📁 Checking project configuration...")
        project_name: str = self.config.project_name

        try:
            project = self.client.get_project(project_name)
            console.print(
                f"✅ Project '{project_name}' already exists. Setting it as active..."
            )
        except KeyError:
            console.print(f"📁 Registering project '{project_name}'...")
            project = self.client.create_project(
                name=project_name,
                description="Dharma : To answer all philosophy questions.",
            )

        self.client.set_active_project(project.id)
